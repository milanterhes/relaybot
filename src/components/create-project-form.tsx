...
const CreateProjectDialog: React.FC<PropsWithChildren<{ guilds: Guild[] }>> = ({
    ...
    const { execute, result } = useAction(makeProject, {
        onError: (error) => {
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                        <FormField control={form.control} name="name" render={({ field }) => (
                            <FormItem className="grid grid-cols-4 items-center gap-4">
                                ...
                                <FormControl>
                                    <Input {...field} placeholder="My project" className="col-span-3" />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        )} />
                        <FormField control={form.control} name="description" render={({ field }) => (
                            <FormItem className="grid grid-cols-4 items-center gap-4">
                                ...
                                <FormControl>
                                    <Input {...field} placeholder="A short description" className="col-span-3" />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        )} />
